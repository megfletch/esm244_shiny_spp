...
species_data_v2 <- read_csv(here("data", "species.csv")) %>% 
  ...
  select(park_name, common_names, category)
...
#combining park data  with park boundary data
park_data_bound <- merge(park_data_v2, park_boundaries, by = "unit_code")

#combining the above combined dataset with the species data set by the name of the park
species_park_bound <- merge(park_data_bound, species_data_v2, by = "park_name")
